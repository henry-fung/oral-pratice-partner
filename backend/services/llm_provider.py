...
class LLMProvider(ABC):
    ...
    def generate(self, messages: List[Dict[str, str]], json_mode: bool = False, response_format: Optional[type] = None) -> str:
        ...
        Args:
            ...
            json_mode: If True, request JSON output (legacy, use response_format instead)
            response_format: Pydantic model class for structured output (Pydantic v2)
...
class GeminiProvider(LLMProvider):
    ...
    def generate(self, messages: List[Dict[str, str]], json_mode: bool = False, response_format: Optional[type] = None) -> str:
        ...
        # Gemini 不支持 Pydantic schema，但可以设置 JSON mime type
        if response_format:
